...
class collapsible_widget_area {
	...
	function admin_enqueue_scripts() {
		wp_register_script( 'collapsible-widgets-admin', plugins_url( 'scripts/collapsible-widgets-admin.js', __FILE__ ), array(
			'jquery',
			'admin-widgets'
		), $this->version, true );
	}
	...
	function add_settings_fields() {
		add_settings_field( 'uitheme', __( 'Theme to use:', 'collapsible-widget-area' ), array(
			$this,
			'do_settings_field'
		), $this->settings_page, 'collapsible_widgets_section', array(
			'label_for'  => 'uitheme',
			'field_name' => 'uitheme'
		) );
		add_settings_field( 'collapsible-sidebars', __( 'Number of collapsible areas to create?', 'collapsible-widget-area' ), array(
			$this,
			'do_settings_field'
		), $this->settings_page, 'collapsible_widgets_section', array(
			'label_for'  => 'sidebars',
	...
	function do_settings_field( $args ) {
		...
		switch ( $args['field_name'] ) {
			case 'uitheme':
				...
				?>
                <select name="collapsible-widget-options[<?php echo $args['field_name'] ?>]"
                        id="<?php echo $args['label_for'] ?>" class="widefat">
			...
			case 'sidebars' :
				?>
                <input type="number" name="collapsible-widget-options[<?php echo $args['field_name'] ?>]"
                       id="<?php echo $args['label_for'] ?>" value="<?php echo (int) $this->options['sidebars'] ?>"
                       class="widefat"/>
				<?php
				break;
		}
	...
	function add_options_page() {
		add_settings_section( 'collapsible_widgets_section', __( 'Collapsible Widget Area Settings', 'collapsible-widget-area' ), array(
			$this,
			'settings_section'
		), $this->settings_page );
		$this->add_settings_fields();
		register_setting( 'collapsible_widgets_section', 'collapsible-widget-options', array(
			$this,
			'sanitize_settings'
		) );

		$args = array(
			__( 'Collapsible Widget Options', 'collapsible-widget-area' ),
			__( 'Collapsible Widget Options', 'collapsible-widget-area' ),
			'manage_options',
			$this->settings_page,
	...
	function admin_options_page() {
        ...
        <div class="wrap">
			...
			if ( $this->_is_mn_settings_page ) {
				...
				?>
                <p>
                    <em><?php _e( 'This is the settings page for the entire multi-network installation. Any options set here will act as the default for all sites; but can be overridden at the individual network level <strong>and</strong> at the individual site level.', 'collapsible-widget-area' ) ?></em>
                </p>
				<?php
				...
				if ( ! $this->is_multinetwork ) {
					?>
                    <p>
                        <em><?php _e( 'This is the settings page for the entire network. Any options set here will act as the default for all sites; but can be overridden at the individual site level.', 'collapsible-widget-area' ) ?></em>
                    </p>
					<?php
				}
			} else {
            ...
            <form name="collwidopts" action="<?php echo $action ?>" method="post">
                ...
                <p><input type="submit" value="<?php _e( 'Save', 'collapsible-widget-area' ) ?>"
