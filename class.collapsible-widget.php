...
class collapsible_widget extends WP_Widget {
	...
	function enqueue_scripts() {
		...
		wp_register_script( 'collapsible-widgets', plugins_url( 'scripts/collapsible-widgets.js', __FILE__ ), array(
			'jquery-cookie',
			'jquery-ui-accordion'
		), $this->version, true );
	...
	function form( $instance ) {
		...
		if ( $instance['invalid-widget'] ) {
            ...
            <div class="hidden"><input type="text" name="<?php echo $this->get_field_name( 'title' ) ?>"
                                       id="<?php echo $this->get_field_id( 'title' ) ?>"
                                       value="<?php _e( 'Error - Attention Required', 'collapsible-widget-area' ) ?>"
                                       readonly/></div>
			<?php
			return;
		}
		...
		?>
        <p>
            <label for="<?php echo $this->get_field_id( 'title' ) ?>"><?php _e( 'Title:', 'collapsible-widget-area' ) ?></label><br/>
            <input type="text" name="<?php echo $this->get_field_name( 'title' ) ?>"
                   id="<?php echo $this->get_field_id( 'title' ) ?>" value="<?php echo $instance['title'] ?>" readonly/><br/>
            <em><?php _e( 'The title is used only to differentiate between instances, and is automatically generated based on the collapsible widget area you choose below. It does not show up on the front-end anywhere.', 'collapsible-widget-area' ) ?></em>
        </p>
        <hr/>
        <p>
            <label for="<?php echo $this->get_field_id( 'sidebar_id' ) ?>"><?php _e( 'Which collapsible widget area should be used?', 'collapsible-widget-area' ) ?></label><br/>
            <select name="<?php echo $this->get_field_name( 'sidebar_id' ) ?>"
                    id="<?php echo $this->get_field_id( 'sidebar_id' ) ?>">
				<?php
        ...
        <p><?php _e( 'Display widgets in which manner?', 'collapsible-widget-area' ) ?><br/>
            <input type="radio" name="<?php echo $this->get_field_name( 'show_what' ) ?>"
                   id="<?php echo $this->get_field_id( 'show_what_tabbed' ) ?>"
                   value="tabbed"<?php checked( $instance['show_what'], 'tabbed' ) ?>/> <label
                    for="<?php echo $this->get_field_id( 'show_what_tabbed' ) ?>"><?php _e( 'Tabs', 'collapsible-widget-area' ) ?></label><br/>
            <input type="radio" name="<?php echo $this->get_field_name( 'show_what' ) ?>"
                   id="<?php echo $this->get_field_id( 'show_what_accordion' ) ?>"
                   value="accordion"<?php checked( $instance['show_what'], 'accordion' ) ?>/> <label
                    for="<?php echo $this->get_field_id( 'show_what_accordion' ) ?>"><?php _e( 'Accordion', 'collapsible-widget-area' ) ?></label>
        </p>
        <p>
			<?php _e( 'How should the panel heights be handled?', 'collapsible-widget-area' ) ?><br/>
            <input type="radio" name="<?php echo $this->get_field_name( 'heightStyle' ) ?>"
                   id="<?php echo $this->get_field_id( 'heightStyle_auto' ) ?>"
                   value="auto"<?php checked( $instance['heightStyle'], 'auto' ) ?>/>
            ...
            <input type="radio" name="<?php echo $this->get_field_name( 'heightStyle' ) ?>"
                   id="<?php echo $this->get_field_id( 'heightStyle_fill' ) ?>"
                   value="fill"<?php checked( $instance['heightStyle'], 'fill' ) ?>/>
            ...
            <input type="radio" name="<?php echo $this->get_field_name( 'heightStyle' ) ?>"
                   id="<?php echo $this->get_field_id( 'heightStyle_content' ) ?>"
                   value="content"<?php checked( $instance['heightStyle'], 'content' ) ?>/>
        ...
        <p><input type="checkbox" name="<?php echo $this->get_field_name( 'collapsible' ) ?>"
                  id="<?php echo $this->get_field_id( 'collapsible' ) ?>"
                  value="1"<?php checked( $instance['collapsible'] ) ?>/> <label
                    for="<?php echo $this->get_field_id( 'collapsible' ) ?>"><?php _e( 'Allow all panels to be closed at once?', 'collapsible-widget-area' ) ?></label>
        </p>
        <p><input type="checkbox" name="<?php echo $this->get_field_name( 'closed' ) ?>"
                  id="<?php echo $this->get_field_id( 'closed' ) ?>" value="1"<?php checked( $instance['closed'] ) ?>/>
            <label for="<?php echo $this->get_field_id( 'closed' ) ?>"><?php _e( 'Start with all panels closed? (only applicable if the above option is checked)', 'collapsible-widget-area' ) ?></label>
        </p>
        <p><input type="checkbox" name="<?php echo $this->get_field_name( 'cookie' ) ?>"
                  id="<?php echo $this->get_field_id( 'cookie' ) ?>" value="1"<?php checked( $instance['cookie'] ) ?>/>
            <label for="<?php echo $this->get_field_id( 'cookie' ) ?>"><?php _e( 'Persist active panel across page views?', 'collapsible-widget-area' ) ?></label>
        </p>
		<?php
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( array_key_exists( 'collapsible', $new_instance ) && in_array( $new_instance['collapsible'], array(
				'1',
				1,
				true
			), true ) ) {
		...
		if ( array_key_exists( 'closed', $new_instance ) && in_array( $new_instance['closed'], array(
				'1',
				1,
				true
			), true ) ) {
		...
		if ( array_key_exists( 'cookie', $new_instance ) && in_array( $new_instance['cookie'], array(
				'1',
				1,
				true
			), true ) ) {
	...
	function widget( $args, $instance ) {
		...
		$this->instance[] = array(
			...
			'heightStyle' => in_array( $instance['heightStyle'], array(
				'fill',
				'content'
			) ) ? $instance['heightStyle'] : 'auto',
